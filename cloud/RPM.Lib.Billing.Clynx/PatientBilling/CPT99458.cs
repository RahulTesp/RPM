...
namespace RPMPatientBilling.PatientBilling
{
    public class CPT99458 : IBilling
    {
        ...
        public CPT99458(string connectionString)
        {
            ...
            billingCode457 = billing.GetBillingCodeDetails(con).Where(s => s.BillingCode == BillingCode457).FirstOrDefault();
        ...
        //    if (TotalReading > (billingCode457.TargetReadings * 60))
        //    {
        //        remainingREading = TotalReading - (billingCode457.TargetReadings * 60);
        //    }
        //    SetData(patientProgramData, remainingREading,
        //            Convert.ToDateTime(stDateTemp), billingCode.BillingCodeID,
        //            DaysCompleted, bCycleComplete, null);
        //    if (bCycleComplete)
        //    {
        ...
        private void AddBilledData(PatientProgramData patientProgramData, DateTime startDate, DateTime endDate, DateTime stDateTemp, DateTime NextEndDate, bool bCycleComplete, int DaysCompleted)
        {
            ...
            if (Dates.Count>0)
            {
                ...
                foreach (Dates date in Dates)
                {
                    ...
                    patientSmsCount =patientSmsCount+billing.GetPatientSmsCount(patientProgramData, con, date.StartDate, date.EndDate);
            ...
            SetData(patientProgramData, remainingREading,
                    Convert.ToDateTime(startDateLoc), billingCode.BillingCodeID,
                    DaysCompleted+1, bCycleComplete, null);
            if (bCycleComplete)
            {
                PatientDailyBillingData patientDailyBillingData = new PatientDailyBillingData()
                {
