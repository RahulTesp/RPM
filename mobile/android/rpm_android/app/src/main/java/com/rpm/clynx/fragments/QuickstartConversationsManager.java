...
void sendMessage(String messageBody, Context context, String ConversationSid, String token, String userId, String toUser, MessageActivity.MessagesAdapter messagesAdapter) {
         Log.d("messageBody1", messageBody);
         if (conversation != null) {
             ...
             Log.d("sendMessage fallback", "Conversation is null. Creating and joining conversation...");

             ConversationsClient convClientVal = ConversationsClientManager.getInstance().getConvClient();

             createAndJoinConversation(toUser, convClientVal, context, token,
                     messagesAdapter, messageBody, token, userId);
         }
     }
             ...
             for (Conversation conversation : conversationList) {
                 ...
                 conversation.getLastMessages(1, new CallbackListener<List<Message>>() {
                     ...
                     public void onSuccess(List<Message> messages) {
                         if (!messages.isEmpty()) {
                             Message lastMessage = messages.get(0);
                             Log.d("InitLastMessage", "Initial last message: " + lastMessage.getBody());
                     ...
                     @Override
                     public void onError(ErrorInfo errorInfo) {
                         Log.e("InitLastMessage", "Error fetching initial last message: " + errorInfo.toString());
                     }
                 });
     ...
     private void updateLastMessageUI(Conversation conversation, Message lastMessage) {
         ...
         if (conversation == null) {
             Log.e("handleUnreadMessages", "Conversation is null");
             return;
         }
         ...
         if (chatListAdapter == null) {
             ...
             Log.d("conversation.getFriendlyName():-", conversation.getFriendlyName());
             Log.d("conversation.getsid():-", conversation.getSid());
             ...
             Log.d("friendlyNamenew:-", friendlyName);
         // Check if conversation has a last message date
             ...
             String formattedTime = (lastMessageDate != null) ?
                 ...
                 "Just Now"; // Default to "Just Now" for new conversations
             String convSid = conversation.getSid();
             // Handle unread message count safely
         int unreadMessages = (unreadMsgCount != null) ? unreadMsgCount.intValue() : 0;

         // If unread count is 0 but a new message is being sent, ensure it's marked as unread
         if (unreadMessages == 0) {
             String finalFriendlyName = friendlyName;
             conversation.getUnreadMessagesCount(new CallbackListener<Long>() {
                 @Override
                 public void onSuccess(Long newUnreadMsgCount) {
                     Log.d("Unread Count Updated", "Unread messages for " + finalFriendlyName + ": " + newUnreadMsgCount);
                     int unreadCount = (newUnreadMsgCount != null) ? newUnreadMsgCount.intValue() : 0;
                     updateChatList(friendlyUserName, finalFriendlyName, formattedTime, unreadCount, timestamp, context,lastMessageText, convSid);
                 }

                 @Override
                 public void onError(ErrorInfo errorInfo) {
                     Log.e("UnreadCountError", "Failed to fetch unread count: " + errorInfo.toString());
                 }
             });
         } else {
             updateChatList(friendlyUserName,friendlyName, formattedTime, unreadMessages, timestamp, context, lastMessageText,  convSid);
         }
     }

     private void updateChatList(String friendlyUserName,String friendlyName, String formattedTime, int unreadMessages, long timestamp, Context context, String lastMessageText, String sid) {
...
// Print sorted chat names to confirm sorting
     ...
     public void createAndJoinConversation(String TO_USER, ConversationsClient convClient, Context context, String convToken, MessageActivity.MessagesAdapter messagesAdapter, String messageBody, String token, String userId) {
        ...
        // Patent_Id = pref.getString("Patent_id", null); // Get logged-in user ID
         ...
         convClient.createConversation(createFriendlyName, new CallbackListener<Conversation>() {
             ...
             public void onSuccess(Conversation conversation) {
                 ...
                 if (conversation != null) {
                     Log.d(DashboardFragment.TAG, "conversation != null " + TO_USER);
                     joinConversationAndSendMessage(conversation, TO_USER, messagesAdapter, messageBody, context, conversation.getSid(),token, userId);
             ...
             @Override
             public void onError(ErrorInfo errorInfo) {
                 Log.e("Error creating conversation:", "Error creating conversation: " + errorInfo.getMessage());
     ...
     private void joinConversation(final Conversation conversation, String TO_USER, MessageActivity.MessagesAdapter messagesAdapter) {
         Log.d(DashboardFragment.TAG, "Joining Conversation: " + conversation.getUniqueName());
         if (conversation.getStatus() == Conversation.ConversationStatus.JOINED) {
             Log.d("conversationgetStatus(", String.valueOf(conversation.getStatus()));
             QuickstartConversationsManager.this.conversation = conversation;
             Log.d(DashboardFragment.TAG, "Already joined default conversation");
             QuickstartConversationsManager.this.conversation.addListener(mDefaultConversationListener);
             return;
         }
         conversation.addParticipantByIdentity(TO_USER,null, new StatusListener() {
             ...
             public void onSuccess() {
                 Log.d("successaddParticipantByIdentity", TO_USER);
             }
         });
         ...
         conversation.join(new StatusListener() {
             ...
             public void onSuccess() {
                 QuickstartConversationsManager.this.conversation = conversation;
                 Log.d(DashboardFragment.TAG, "Joined default conversation");
                 QuickstartConversationsManager.this.conversation.addListener(mDefaultConversationListener);
                 QuickstartConversationsManager.this.loadPreviousMessages(conversation,messagesAdapter);
             }
     ...
     private void joinConversationAndSendMessage(final Conversation conversation, String TO_USER, MessageActivity.MessagesAdapter messagesAdapter, String messageBody, Context context, String conversationSid, String token, String userId) {
        Log.d(DashboardFragment.TAG, "Joining Conversation: " + conversation.getUniqueName());
        if (conversation.getStatus() == Conversation.ConversationStatus.JOINED) {
            Log.d("conversationgetStatus(", String.valueOf(conversation.getStatus()));
            QuickstartConversationsManager.this.conversation = conversation;
