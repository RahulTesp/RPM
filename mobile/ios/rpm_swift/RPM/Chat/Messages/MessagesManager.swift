...
class MessagesManager: ObservableObject {
    ...
    func deleteAndRemoveFromStore(_ message: PersistentMessageDataItem, completion: @escaping (Bool) -> Void) {
        guard let conversationSid = message.conversationSid else {
            completion(false)
            return
        }

        // Optimistically remove from memory and Core Data
        if let sid = message.sid {
            self.messages.removeAll { $0.sid == sid }
        }
        self.coreDataDelegate.managedObjectContext.delete(message)
        self.coreDataDelegate.saveContext()

        // Proceed with Twilio deletion
        deleteMessageFromTwilio(message) { [weak self] success in
            guard let self = self else {
                completion(false)
                return
            }

            // If deletion failed, try to restore
            if !success {
                self.conversationManager.retrieveConversation(conversationSid) { tchConversation, error in
                    guard let conversation = tchConversation else {
                        completion(false)
                        return
                    }

                    let index = NSNumber(value: message.messageIndex)

                    conversation.message(withIndex: index) { result, tchMessage in
                        guard let twilioMessage = tchMessage else {
                            completion(false)
                            return
                        }

                        if let restored = PersistentMessageDataItem.from(
                            message: twilioMessage,
                            inConversation: conversation,
                            withDirection: twilioMessage.author == AppModel.shared.myIdentity ? .outgoing : .incoming,
                            inContext: self.coreDataDelegate.managedObjectContext
                        ) {
                            self.messages.append(restored)
                            self.coreDataDelegate.saveContext()
                            print(" Restored message after failed deletion")
                        }

                        completion(false) // false because deletion ultimately failed
                    }
                }
            } else {
                completion(true) //  deletion succeeded
            }
        }
    }
    ...
    // Step 1: Add completion to updateMessageBody
    func updateMessageBody(newText: String, for messageIndex: Int64?, conversationSid: String?, completion: ((Bool) -> Void)? = nil) {
        guard let conversationSid = conversationSid,
              let messageIndex = messageIndex else {
            completion?(false)
            return
