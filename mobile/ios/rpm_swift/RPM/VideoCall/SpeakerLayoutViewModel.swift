...
class SpeakerLayoutViewModel: ObservableObject {
    ...
    private func update() {
        let newSpeaker = findDominantSpeaker()
        
        // Instead of replacing, update the existing view model
        dominantSpeaker.identity = newSpeaker.identity
        dominantSpeaker.displayName = newSpeaker.displayName
        dominantSpeaker.isYou = newSpeaker.isYou
        dominantSpeaker.isMuted = newSpeaker.isMuted
        dominantSpeaker.dominantSpeakerStartTime = newSpeaker.dominantSpeakerStartTime
        dominantSpeaker.isDominantSpeaker = false
        dominantSpeaker.cameraTrack = newSpeaker.cameraTrack
        dominantSpeaker.shouldMirrorCameraVideo = newSpeaker.shouldMirrorCameraVideo
        dominantSpeaker.shouldFillCameraVideo = newSpeaker.shouldFillCameraVideo
        dominantSpeaker.networkQualityLevel = newSpeaker.networkQualityLevel

        if let presenter = findPresenter() {
            self.presenter = presenter
            if !isPresenting { isPresenting = true }
        } else {
            presenter = Presenter()
