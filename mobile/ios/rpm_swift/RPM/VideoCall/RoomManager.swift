...
class RoomManager: NSObject, ObservableObject {
    ...
    @Published var remoteParticipantViewModels: [ParticipantViewModel] = []
...
extension RoomManager: RoomDelegate {
    ...
    func roomDidConnect(room: Room) {
        ...
        if let sdkLocalParticipant = room.localParticipant {
            print("Local Participant: \(sdkLocalParticipant.identity)")

            sdkLocalParticipant.localAudioTracks.forEach { publication in
                print("Published Audio Track: \(publication.track?.name ?? "Unknown")")
            }

            sdkLocalParticipant.localVideoTracks.forEach { publication in
                print("Published Video Track: \(publication.track?.name ?? "Unknown")")
            }

            //  Assign the SDK participant to your app's localParticipant manager
            localParticipant.participant = sdkLocalParticipant

            //  Now re-apply the mic/camera settings on your LocalParticipantManager
            if localParticipant.isMicOn {
                localParticipant.isMicOn = true // Re-triggers the didSet
            }
    ...
    func roomDidDisconnect(room: Room, error: Error?) {
        if let error = error {
            print("roomdisconecterror: \(error)")
            handleError(error)
        }
    }
...
extension RoomManager: RemoteParticipantManagerDelegate {
    ...
    func participantDidChange(_ participant: RemoteParticipantManager) {
        ...
        if let cameraTrack = participant.cameraTrack {
            print("participantDidChange remote video track: \(participant.description), isEnabled: \(cameraTrack.isEnabled)")
        } else {
            print("participantDidChange remote video track: \(participant.description), isEnabled: nil")
        }

        // No early return here — notify UI regardless of cameraTrack or enabled state
        
        // Additional logs for clarity
        ...
        let isVideoMuted = participant.isVideoMuted
        let isVideoEnabled = participant.cameraTrack?.isEnabled ?? false
        
        print("Video enabled: \(isVideoEnabled), Video muted: \(isVideoMuted)")

        // Send update to UI subscribers
