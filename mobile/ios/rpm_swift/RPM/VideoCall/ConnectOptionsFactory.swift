...
class ConnectOptionsFactory: NSObject {
    ...
    func makeConnectOptions(
        ...
        ConnectOptions(token: accessToken) { builder in
            ...
            print("Audio Tracks: \(audioTracks.map { $0.name })")
            print("Video Tracks: \(videoTracks.map { $0.name })")

            builder.audioTracks.forEach { track in
                print("Added Audio Track: \(track.name)")
            }

            builder.videoTracks.forEach { track in
                print("Added Video Track: \(track.name)")
            }
